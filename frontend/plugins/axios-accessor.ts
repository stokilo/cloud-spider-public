...
import { ROUTE_NAMES, UNAUTHENTICATED_ROUTE_NAMES } from '@backend/routes'
import { initializeAxios, $token, $log } from '~/utils/api'
...
const accessor: Plugin = (context) => {
  ...
  context.$axios.interceptors.request.use(
    async (config) => {
      if (
        config.url &&
        config.url !== ROUTE_NAMES.OAUTH_REFRESH_TOKEN &&
